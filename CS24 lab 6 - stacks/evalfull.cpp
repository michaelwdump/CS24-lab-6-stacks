...
bool balanced(char *expression[], int numTokens) {
    //iteratue thru c-string and push every "("
    //if ")" found, pop off a "(".
        //if first thing found is a "(", return false
        //if empty(), and "(" found, return false
    ...
    for (int i=0; i < numTokens; ++i) {
        //FIXME: the '(' and ')' should be const char pointers, not characters. find a way to work that out.
        if (strcmp(expression[i], leftPP)) {
            s.push(leftPP);
        }
        if (strcmp(expression[i], rightPP)) {
            if (!s.empty()) { 
                s.pop();
            }
            else return false; //s is empty and an extra ')' was found
        }
